namespace BSG.ADInventory.Services
{
    ...
    public class InvDocService : CrudService<InvDoc>, IInvDocService
    {
        ...
        public InvDocService(IUnitOfWork unitOfWork, IInvDocRepository invDocRepository, IInvDocDetailRepository invDocDetailRepository,
            ...
            IPurchaseDocRepository purchaseDocRepository, ISqlCommandRepository sqlCommandRepository)
        ...
        public IEnumerable<InvDocListModel> GetDataList(long docType)
        {
            ...
            try
            {
                ...
                var DocType = new SqlParameter { ParameterName = "@DocType", Direction = ParameterDirection.Input, Value = docType, DbType = DbType.Int64 };                


                List<InvDocListModel> data = new List<InvDocListModel>();
                try
                {
                    data = (from r in sqlCommandRepository.ExecuteStoredProcedureList<InvDocListModel>("SP_GetInvDocDataList @DocType",DocType)
                            select new InvDocListModel
                            {
                                Id = r.Id,
                                InvDocType = r.InvDocType,
                                DocNo = r.DocNo,
                                InvDocStatus = r.InvDocStatus,
                                ReferenceInternalDocNo = r.ReferenceInternalDocNo,
                                CarType = r.CarType,
                                CarPlateNumer = r.CarPlateNumer,
                ...
                // Before Link To Tadbir

                //var data = this.invDocRepository.Query
                //    .Where(d => d.InvDocTypeId == docType && userProjects.Contains(d.ProjectId) && d.IsAggregated==false).AsEnumerable()
                ...
                //        PurchaseDocId=d.PurchaseDocId,
                //        Source =docType==6?(d.SourceProject.Township.Title+"-"+d.SourceProject.Title) :( docType == 1 ? (d.Provider == null ? "" : d.Provider.Township.Title + "-" + d.Provider.Title) : (docType == 2 ? d.Project.Title : string.Empty)),
                //        Dest = docType == 6 ? (d.DestProject.Township.Title + "-" + d.DestProject.Title) : (docType == 7 ? d.Provider.Township.Title + "-" + d.Provider.Title : string.Empty),
                ...
                //        ItemsSummary = String.Join(",", d.InvDocDetails.Select(dt => dt.Mat.Title).Distinct().ToArray()),
        ...
        public CustomResult<bool> SetEntranceDocData(EntranceDocDataModel dataModel)
        {
            ...
            // Check PurchaseDocId
            if (dataModel.PurchaseDocId != null && !this.purchaseDocRepository.Query.Where(p => p.Id == dataModel.PurchaseDocId && p.IsFinished == false).Any())
            {
                data.Result = false;
                data.ErrorMessage = Resource.PurchaseDocIdIsNotValid;
                return data;
            }
            ...
            if (dataModel.Id > 0)
            {
                ...
                invDoc.PurchaseDocId = dataModel.PurchaseDocId;
            ...
            else
            {
                ...
                var doc = this.invDocRepository.Query.Where(d => d.FinanceYear == dataModel.FinanceYear && d.InvDocTypeId==1).Max(d => d.DocNo);
                string MaxId = "";
                if (doc == null)
                {
                    MaxId = "A"+dataModel.FinanceYear.ToString() + "000001";
                }
                else
                {
                ...
                invDoc = new InvDoc
                {
                    ...
                    PurchaseDocId = dataModel.PurchaseDocId
                };
        ...
        public CustomResult<bool> SetInternalEntranceDocData(InternalEntranceDocDataModel dataModel)
        {
            ...
            else
            {
                ...
                dataModel.FinanceYear = finYear;
                var doc = this.invDocRepository.Query.Where(d => d.FinanceYear == dataModel.FinanceYear && d.InvDocTypeId==2).Max(d => d.DocNo);
                string MaxId = "";
                if (doc == null)
                {
        ...
        public CustomResult<bool> SetUseDocData(UseDocDataModel dataModel)
        {
            ...
            else
            {
                ...
                dataModel.FinanceYear = finYear;
                var doc = this.invDocRepository.Query.Where(d => d.FinanceYear == dataModel.FinanceYear && d.InvDocTypeId==4).Max(d => d.DocNo);
                string MaxId = "";
                if (doc == null)
                {
                    MaxId = "D" + dataModel.FinanceYear.ToString() + "000001";
                }
                else
                {
                    MaxId = Common.Extention.Numberic.NextInvDocId(doc);
        ...
        public CustomResult<bool> SetReturnDocData(ReturnDocDataModel dataModel)
        {
            ...
            else
            {
                ...
                dataModel.FinanceYear = finYear;
                var doc = this.invDocRepository.Query.Where(d => d.FinanceYear == dataModel.FinanceYear && d.InvDocTypeId==5).Max(d => d.DocNo);
                string MaxId = "";
                if (doc == null)
                {
                    MaxId ="E"+ dataModel.FinanceYear.ToString() + "000001";
                }
                else
                {
                    MaxId = Common.Extention.Numberic.NextInvDocId(doc);
        ...
        public CustomResult<bool> SetExternalBOLDocData(ExternalBOLDocDataModel dataModel)
        {
            ...
            else
            {
                ...
                dataModel.FinanceYear = finYear;
                var doc = this.invDocRepository.Query.Where(d => d.FinanceYear == dataModel.FinanceYear && d.InvDocTypeId==7).Max(d => d.DocNo);
                string MaxId = "";
                if (doc == null)
                {
                    MaxId = "G"+dataModel.FinanceYear.ToString() + "000001";
                }
                else
                {
                    MaxId = Common.Extention.Numberic.NextInvDocId(doc);
        ...
        public CustomResult<bool> SetInternalBOLDocData(InternalBOLDocDataModel dataModel)
        {
            ...
            if (dataModel.Id > 0)
            {
                ...
                invDoc = oldDoc;
                invDoc.ProjectId = dataModel.ProjectId;
                invDoc.SourceProjectId = dataModel.ProjectId;
            ...
            else
            {
                ...
                dataModel.FinanceYear = finYear;
                var doc = this.invDocRepository.Query.Where(d => d.FinanceYear == dataModel.FinanceYear && d.InvDocTypeId==6).Max(d => d.DocNo);
                string MaxId = "";
                if (doc == null)
                {
                    MaxId = "F"+dataModel.FinanceYear.ToString() + "000001";
                }
                else
                {
                    MaxId = Common.Extention.Numberic.NextInvDocId(doc);
                ...
                invDoc = new InvDoc
                {
                    ...
                    DocNo = MaxId,
                    ProjectId = dataModel.ProjectId,
                    SourceProjectId = dataModel.ProjectId,                    
        ...
        public CustomResult<bool> PurchaseDocValidation(long id)
        {
            CustomResult<bool> data =new  CustomResult<bool>();

            var invdoc = this.invDocRepository.GetItemByKey(id);
            if (invdoc != null)
            {
                //if (invdoc.InvDocTypeId==1 && invdoc.PurchaseDocId == null && !(invdoc.InvDocDetails.Where(m => m.PurchaseDocId != null).Any()))
                //{
                //    data.Result = false;
                //    data.ErrorMessage = Resource.TherIsNoPurchaseDocForDocOrDetails;
                //}

                //else if (invdoc.InvDocTypeId == 1 && invdoc.PurchaseDoc != null && (invdoc.InvDocDetails.Where(m => m.PurchaseDocId != null).Any()))
                //{
                //    data.Result = false;
                //    data.ErrorMessage = Resource.PurchaseDocShouldBeAssignToDocOrDetails;
                //}
                //else if (invdoc.InvDocTypeId == 1 && invdoc.PurchaseDoc == null && (invdoc.InvDocDetails.Where(m => m.PurchaseDocId == null).Any()))
                //{
                //    data.Result = false;
                //    data.ErrorMessage = Resource.PurchaseDocShouldBeAssignToAllDetails;
                //}
                //else
                //{
                    data.Result = true;
        ...
        //public IEnumerable<InvDocListModel> GetDataListCentral(long docType)
        //{
        //    var userProjects = this.userRepository.GetCurrentUser().Projects.Select(p => p.Id).ToList();
        //    try
        //    {
        //        var data = this.invDocRepository.Query
        //            .Where(d => d.InvDocTypeId == docType && userProjects.Contains(d.ProjectId) && d.IsAggregated == true).AsEnumerable()
        //            .OrderByDescending(d => d.Id)
        //            .Select(d => new InvDocListModel
        //            {
        //                Id = d.Id,
        ...
        //public CustomResult<bool> SetCentralEntranceDocData(EntranceDocDataModel dataModel)
        //{
        //    CustomResult<bool> data = new CustomResult<bool>();

        //    // Check PurchaseDocId
        //    if (dataModel.PurchaseDocId != null && !this.purchaseDocRepository.Query.Where(p => p.Id == dataModel.PurchaseDocId && (p.IsFinished == false || p.IsAggregated==false) ).Any())
        //    {
        //        data.Result = false;
        //        data.ErrorMessage = Resource.PurchaseDocIdIsNotValid;
        //        return data;
        ...
        //    //else if (dataModel.PurchaseDocId != null && dataModel.Mats.Where(m => m.PurchaseDocId != null).Any())
        //    //{
        //    //    data.Result = false;
        //    //    data.ErrorMessage = Resource.PurchaseDocShouldBeAssignToDocOrDetails;
        //    //    return data;
        ...
        //    //foreach (var item in dataModel.Mats.Where(m => m.PurchaseDocId != null))
        //    //{

        //    //    if (!this.purchaseDocRepository.Query.Where(p => p.Id == item.PurchaseDocId && (p.IsFinished == false || p.IsAggregated==false)).Any())
        //    //    {
        //    //        data.Result = false;
        //    //        data.ErrorMessage = Resource.PurchaseDocIdIsNotValidForDocDetail;
        ...
        //    InvDoc invDoc = new InvDoc { IsAggregated=true};
        //    var invDocType = this.invDocTypeRepository.GetItemByKey(dataModel.InvDocTypeId);
        ...
        //public CustomResult<bool> SetCentralInternalBOLDocData(InternalBOLDocDataModel dataModel)
        //{
        //    CustomResult<bool> data = new CustomResult<bool>();
        //    InvDoc invDoc = new InvDoc { IsAggregated=true};
        //    var invDocType = this.invDocTypeRepository.GetItemByKey(dataModel.InvDocTypeId);
