namespace BSG.ADInventory.Services
{
    ...
    public class CarService : CrudService<Car>, ICarService
    {
        ...
        public IEnumerable<CarListModel> GetDataList()
        {
            var data = this.carRepository.GetAllItems().Select(c => new CarListModel
            {
                Id = c.Id,
                Title = c.Title,
                CarType = c.CarType.Title,
                PlateNumber = "ایران"+c.PlateSeries+" | "+c.PlateNumberPart2+c.PlateCharacter.Title+c.PlateNumberPart1,
                IsActive = c.IsActive,
                ModelName = c.ModelName,
                ProductDate = c.ProductDate==null?"":Common.DateTimeHelper.ToStringDate(c.ProductDate),
                Description = c.Description,                
                Driver = c.DriverId == null ? string.Empty : c.Driver.FirstName + " " + c.Driver.LastName
